...
BEGIN
FOR "routeIterate" IN SELECT "MonitoringKey" FROM "TradingRoute"
LOOP
...
CREATE OR REPLACE VIEW "Hops" AS
	SELECT p1."PlanetID" AS "Origin", p1."VisitOrder"
	FROM "PortsOfCall" p1
...
FOR "hopIterate" IN SELECT "Origin" FROM "Hops"
LOOP
-- 15) Within the loop over hops, get the partial distances of the hop. 
-- "query" := 'SELECT "AvgDistance" '
-- || 'FROM "Distance" '
-- || 'WHERE "PlanetOrigin" = ' || "hopIterate"."Origin" 
-- || ' AND "PlanetOrigin" <> "PlanetDestination"';
"query" := 'SELECT "AvgDistance" '
|| 'FROM "Distance" '
|| 'WHERE "PlanetOrigin" = ' || "hopIterate"."Origin";
